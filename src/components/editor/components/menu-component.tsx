...
import { Form, Input, Menu, theme as antdTheme } from 'antd';
...
MenuComponent = ({ theme, ...props }: TextProp) => {
  ...
  const buildMenu = (parentId: string) => {
    ...
    return children.map((x: any) => {
      ...
      return grandChildren && grandChildren.length > 0 && child.pageType !== 'Listing' ? (
  ...
  const topMenu = () => {
    const root = pageLayouts.find((x: any) => x.parent === 'ROOT');
