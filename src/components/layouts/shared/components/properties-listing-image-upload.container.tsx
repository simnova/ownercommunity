...
export const PropertiesListingImageUploadContainer: React.FC<PropertiesListingImageUploadContainerProps> = (props) => {
  const [membersPropertyListingCreateAuthHeader] = useMutation(
    SharedPropertiesListingImageUploadContainerPropertyListingImageCreateAuthHeaderDocument
  );
  ...
  const handleAuthorizeRequest = async (file: File): Promise<AuthResult> => {
    ...
    return result.data
      ? ({ ...result.data.propertyListingImageCreateAuthHeader, ...{ isAuthorized: true } } as AuthResult)
