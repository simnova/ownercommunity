...
export class MsalApp {
  ...
  public registerCallback = (
    callback: (isLoggedIn: boolean, authResult: msal.AuthenticationResult | undefined) => void
  ) => {
  ...
  public async login(options?: {
    ...
    if (this.usePopup) {
      const popupConfig = this.config as MsalProviderPopupConfig;
      ...
      const redirectConfig = this.config as MsalProviderRedirectConfig;
  ...
  private async loginPopup(loginRequestConfig?: msal.PopupRequest): Promise<msal.AuthenticationResult | undefined> {
    try {
  ...
  public async loginRedirect(redirectRequestConfig?: msal.RedirectRequest | undefined) {
    try {
  ...
  public async handleRedirectResult(authResult: msal.AuthenticationResult | null) {
  ...
  private getAccount(providedHomeAccountId?: string): msal.AccountInfo | undefined {
  ...
  public async getAuthToken(providedHomeAccountId?: string): Promise<string | undefined> {
  ...
  public getAuthResults(
    ...
    const fullSilentRequestConfig = this.getFullSilentRequestConfig(
    ...
    if (this.usePopup) {
      const popupConfig = this.config as MsalProviderPopupConfig;
      return this.authTokenPopup(fullSilentRequestConfig, popupConfig.loginRequestConfig, isSilent);
    } else {
      const redirectConfig = this.config as MsalProviderRedirectConfig;
      return this.authTokenRedirect(fullSilentRequestConfig, redirectConfig?.redirectRequestConfig, isSilent);
    }
  }

  public async getAuthResult(providedHomeAccountId?: string): Promise<msal.AuthenticationResult | undefined> {
    return this.getAuthResults(providedHomeAccountId, false);
  }

  public async getSilentAuthResult(providedHomeAccountId?: string): Promise<msal.AuthenticationResult | undefined> {
  ...
  private async authTokenRedirect(
    ...
    try {
      ...
      let authResult = await this.msalInstance.acquireTokenSilent(silentRequest);
