...
export const PropertiesListSearchTags: React.FC<any> = () => {
  ...
  const removeFilter = (filter: string) => {
    ...
    const searchParamMap: Record<
      string,
      { searchId: string; compare?: (id: string, value: string) => boolean; separator?: string }
    > = {
      'Listed Info': {
    ...
    if (qsValue?.includes(separator)) {
      ...
      const newValue = qsValues?.filter((id: string) =>
        searchParamKey.compare ? !searchParamKey.compare(id, value) : id !== value
      );
