...
import * as ActivityDetailValueObjects from './violation-ticket-activity-detail.value-objects';
...
import { ViolationTicketV1ActivityDetailProps, ViolationTicketV1ActivityDetailEntityReference, ViolationTicketV1ActivityDetail } from './violation-ticket-activity-detail';
import { ViolationTicketV1PhotoProps, ViolationTicketV1PhotoEntityReference, ViolationTicketV1Photo } from './violation-ticket-photo';
...
export interface ViolationTicketV1Props extends EntityProps {
  ...
  status: string;
  priority: number;
  readonly activityLog: PropArray<ViolationTicketV1ActivityDetailProps>;
  readonly photos: PropArray<ViolationTicketV1PhotoProps>;
...
export class ViolationTicketV1<props extends ViolationTicketV1Props> extends AggregateRoot<props> implements ViolationTicketV1EntityReference {
  ...
  get activityLog(): ReadonlyArray<ViolationTicketV1ActivityDetailEntityReference> {
    return this.props.activityLog.items.map((a) => new ViolationTicketV1ActivityDetail(a, this.context, this.visa));
  }
  get photos(): ReadonlyArray<ViolationTicketV1PhotoEntityReference> {
    return this.props.photos.items.map((p) => new ViolationTicketV1Photo(p, this.context, this.visa));
  }
  get createdAt(): Date {
  ...
  private requestNewActivityDetail(): ViolationTicketV1ActivityDetail {
    let activityDetail = this.props.activityLog.getNewItem();
    return new ViolationTicketV1ActivityDetail(activityDetail, this.context, this.visa);
  }
