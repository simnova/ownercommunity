  ...
  return (
    <div style={{ display: 'flex', flex: '1', flexDirection: 'column', alignItems: 'stretch' }}>
      <div style={{ overflow: 'scroll' }}>
        <TreeSelect
          ...
          onChange={(value: any) => {
            ...
            if (node?.layout) {
              pageLayout =
                JSON.parse(JSON.stringify(pageLayouts.find((item: any) => item.id === value)?.layout)) ?? undefined;
        ...
        <MobileOutlined
          ...
          style={{
            fontSize: '20px',
            color: colorTextBase,
            border: mobileView ? `${colorTextBase} solid 3px` : 'none',
            padding: '5px'
          }}
        />
        <DesktopOutlined
        ...
        />
        <Button size="small" onClick={() => setMobileView(!mobileView)}>
          Toggle Mobile View
        </Button>
      </div>
      <div style={{ display: 'flex', flexGrow: 1, height: 0 }}>
        {selectedPage ? (
          <div
            className="bg-slate-500 items-center flex flex-col"
            style={{ flex: '1, auto', overflow: 'auto', border: '1px solid lightgray' }}
          >
            <div
              key={selectedPage.toString()}
              style={{ display: 'flex', flexDirection: 'column' }}
              className={
                'grow overflow-y-auto transition-[max-width] transition-[min-width] ease-in-out duration-150 ' +
                (mobileView ? ' max-w-[375px] max-h-[667px] min-w-[375px] ' : 'max-w-[100%] max-h-[100%] min-w-[100%]')
              }
            >
          ...
          <div className="bg-slate-500 items-center flex flex-col" style={{ flex: 1, alignItems: 'stretch' }}>
            <div style={{ backgroundColor: '#f7f7f7', display: 'flex', alignItems: 'center', flex: '1' }}>
              <Empty style={{ minWidth: '100%' }} description={<span>Select a Page above to edit</span>}></Empty>
            </div>
          </div>
        )}
      </div>
