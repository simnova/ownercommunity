...
export class Maps {
    ...
    constructor(){
        ...
        let credentials = new DefaultAzureCredential( { ManangedIdentityClientId: this.tryGetEnvVar(this._appIdentityKey) } as DefaultAzureCredentialOptions);
    ...
    public async generateSharedKey(): Promise<string> {
        try {
            var sasToken = await this._azureMapsClient.accounts.listSas(this._resourceGroup, this._mapsAccountName, { expiry : dayjs().add(10, 'minutes').toISOString(), maxRatePerSecond: 10, principalId: this._principalId, start: dayjs().subtract(5, 'minutes').toISOString(), signingKey: this._mapKey} as AccountSasParameters); // ,  { expiry : dayjs().add(10, 'minutes').toISOString(), maxRatePerSecond: 10, principalId: "", start: dayjs().subtract(5, 'minutes').toISOString()} as AccountSasParameters
