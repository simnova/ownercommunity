  ...
  Member: {
    ...
    role: async (parent, args, context, info) => {
      if (parent.role && isValidObjectId(parent.role.toString())) {
        return (await context.dataSources.roleApi.findOneById(
  ...
  MemberAccount: {
    user: async (parent, args, context, info) => {
      if (parent.user && isValidObjectId(parent.user.toString())) {
        return (await context.dataSources.userApi.findOneById(
  ...
  Query: {
    member: async (_parent, args, context, info) => {
      if (args.id && isValidObjectId(args.id)) {
        return (await context.dataSources.memberApi.findOneById(
          args.id
        )) as Member;
      }
      return null;
    ...
    membersByCommunityId: async (_, { communityId }, { dataSources }) => {
      return (await dataSources.memberApi.getMembersByCommunityId(
        communityId
      )) as Member[];
    },
    ...
    memberForUser: async (_parent, input, context, info) => {
      return (await context.dataSources.memberApi.getMemberByCommunityIdUserId(
        context.community,
        input.userId
      )) as Member;
    },
  ...
  Mutation: {
    memberCreate: async (_, { input }, { dataSources }) => {
      return MemberMutationResolver(
        dataSources.memberDomainAPI.memberCreate(input)
      );
    },
    memberUpdate: async (_, { input }, { dataSources }) => {
      return MemberMutationResolver(
        dataSources.memberDomainAPI.memberUpdate(input)
      );
    },
    memberAccountAdd: async (_, { input }, { dataSources }) => {
      return MemberMutationResolver(
        dataSources.memberDomainAPI.memberAccountAdd(input)
      );
    },
    memberAccountEdit: async (_, { input }, { dataSources }) => {
      return MemberMutationResolver(
        dataSources.memberDomainAPI.memberAccountEdit(input)
      );
    },
    memberAccountRemove: async (_, { input }, { dataSources }) => {
      return MemberMutationResolver(
        dataSources.memberDomainAPI.memberAccountRemove(input)
      );
    },
    memberProfileUpdate: async (_, { input }, { dataSources }) => {
      return MemberMutationResolver(
        dataSources.memberDomainAPI.memberProfileUpdate(input)
      );
    },
    memberProfileAvatarCreateAuthHeader: async (
      _,
      { input },
      { dataSources }
    ) => {
      var result =
        await dataSources.memberBlobAPI.memberProfileAvatarCreateAuthHeader(
          input.memberId,
          input.contentType,
          input.contentLength
        );
      if (result.status.success) {
        result.member =
          (await dataSources.memberDomainAPI.memberProfileUpdateAvatar(
            input.memberId,
            result.authHeader.blobName
          )) as any;
      }
      return result;
    },
    memberProfileAvatarRemove: async (_, { memberId }, { dataSources }) => {
      let result = {
        status: await dataSources.memberBlobAPI.memberProfileAvatarRemove(
          memberId
        ),
      } as MemberMutationResult;
      ...
      if (!result.status.success) {
        ...
        return MemberMutationResolver(
          dataSources.memberDomainAPI.memberProfileUpdateAvatar(memberId, null)
        );
      }
    },
  },
