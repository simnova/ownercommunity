...
const serviceTicket: Resolvers = {
  ...
  Query: {
    ...
    serviceTicketsOpenByCommunity: async (_parent, _args, context, _info) => {
      return (await context.applicationServices.serviceTicketDataApi.getServiceTicketsByCommunityId(context.community?.id)) as ServiceTicket[];
    },
    ...
    serviceTicketsAssignedToCurrentUser: async (_, _args, context) => {
      const member = await getMemberForCurrentUser(context);
      return (await context.applicationServices.serviceTicketDataApi.getServiceTicketsByAssignedTo(context.community?.id, member.id)) as ServiceTicket[];
    },
    ...
    serviceTicketsSearchAdmin: async (_, { input }, context, info) => {
      const searchResults = await context.applicationServices.serviceTicketSearchApi.serviceTicketsSearchAdmin(input, context.community?.id);
