...
export const PropertiesFloorPlanUploadContainer: React.FC<PropertiesFloorPlanUploadContainerProps> = (props) => {
  const [membersPropertyFloorPlanCreateAuthHeader] = useMutation(
    SharedPropertiesListingImageUploadContainerPropertyFloorPlanImageCreateAuthHeaderDocument
  );
  const handleAuthorizeRequest = async (file: File): Promise<AuthResult> => {
    ...
    return result.data
      ? ({ ...result.data.propertyFloorPlanImageCreateAuthHeader, ...{ isAuthorized: true } } as AuthResult)
