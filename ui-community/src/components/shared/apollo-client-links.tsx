import { ApolloClient, ApolloLink, DefaultContext, HttpLink, InMemoryCache, from } from '@apollo/client';
...
export const TerminatingApolloHttpLinkForGraphqlServer = (config: BatchHttpLink.Options) => {
  ...
  const persistedQueryLink = createPersistedQueryLink({
    sha256,
    useGETForHashedQueries: true // use GET for hashed queries to support caching on front door
  }).concat(link);
  return from([removeTypenameFromVariables(), persistedQueryLink]);
};
