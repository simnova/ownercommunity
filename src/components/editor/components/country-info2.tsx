...
CountryInfo2 = ({ country, ...props }: CountryInfo2Prop) => {
  ...
  const { loading, data, error } = useQuery(GET_COUNTRY_DETAILS, {
    ...
    <div className="px-4 py-2" ref={(ref) => connect(drag(ref as HTMLDivElement))} {...props}>
      {data?.country && (
        ...
        >
          ...
          <div
            className="grid grid-cols-3 bg-white px-4 py-2"
            style={{
              backgroundColor: colorBgContainer
            }}
          >
          ...
          >
            ...
            >
              {data.country.languages.map((l: any) => l.name).join(', ')}
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      <Input
        ...
        value={country}
        onChange={(inputElement) => setProp((props: any) => (props.country = inputElement.target.value))}
      />
    </div>
  );
};
